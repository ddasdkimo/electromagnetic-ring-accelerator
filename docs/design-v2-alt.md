...
兩者不一致，而且錯的方向最不利——**外側比內側還緩**，正好是離心力要壓過去的那一面。若正式件照這個多邊形出圖：

```
側向承載 = tan(35.5°) = 0.71 g  →  v_max = √(0.71×9.8×0.12) = 0.92 m/s
```

比 45° 的 1.08 m/s 再低 15%。這只是概念示意圖、不是製造圖（PLAN.md 的規格寫的是 45°），
但**值得跟 stl session 確認正式件的剖面用的是哪一組數字**——這是無論走 v1 或 v2 都該先釐清的事。
...
| bobbin | 內孔 11.2mm、壁 0.8mm、長 30mm、法蘭 Ø22 | 磁隙 0.85mm/邊 |
